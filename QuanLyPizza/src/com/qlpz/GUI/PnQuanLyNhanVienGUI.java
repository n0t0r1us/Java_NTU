...
import com.qlpz.BLL.PhanQuyenBLL;
import com.qlpz.BLL.TaiKhoanBLL;
...
public class PnQuanLyNhanVienGUI extends JPanel {
    ...
    private void xuLyXoaQuyen() {
        ...
        boolean flag = phanQuyenBLL.xoaQuyen(tenQuyen);
        if (flag) {
            loadDataCmbQuyen();
        }
    }
    ...
    private void xuLyThemQuyen() {
        ...
        boolean flag = phanQuyenBLL.themQuyen(tenQuyen);
        if (flag) {
            loadDataCmbQuyen();
        }
    }
    ...
    private void xuLySuaQuyen() {
        ...
        boolean flag = phanQuyenBLL.suaQuyen(tenQuyen, nhapHang, sanPham, nhanVien, khachHang, thongKe);
        if (flag) {
            loadDataCmbQuyen();
        }
    }
    ...
    private void xuLyHienThiChiTietQuyen() {
        ArrayList<PhanQuyen> dsq = phanQuyenBLL.getListQuyen();
    ...
    private void xuLyKhoaTaiKhoan() {
        ...
        taiKhoanBLL.khoaTaiKhoan(txtMaNV.getText());
        loadDataTblNhanVien();
    }
    ...
    private void xuLyNhapExcel() {
        ...
        for (int i = 0; i < row; i++) {
            ...
            nhanVienBLL.nhapExcel(ho, ten, gioiTinh, chucVu);
    ...
    private void xuLyXoaNhanVien() {
        ...
        boolean flag = nhanVienBLL.xoaNhanVien(ma);
        if (flag) {
            nhanVienBLL.docDanhSach();
            loadDataTblNhanVien();
        }
    }
    ...
    private void xuLySuaNhanVien() {
        ...
        if (nhanVienBLL.updateNhanVien(ma, ho, ten, gioiTinh, chucVu)) {
            nhanVienBLL.docDanhSach();
            loadDataTblNhanVien();
        }
    }
    ...
    private void xuLyThemNhanVien() {
        ...
        String gioiTinh = cmbGioiTinh.getSelectedItem() + "";
        String chucVu = txtChucVu.getText();
        if (nhanVienBLL.themNhanVien(ho, ten, gioiTinh, chucVu)) {
            nhanVienBLL.docDanhSach();
            loadDataTblNhanVien();
        }
    }
    ...
    private void xuLyTimKiemNhanVien() {
        ArrayList<NhanVien> dsnv = nhanVienBLL.timNhanVien(txtTimNV.getText());
    ...
    private void loadDataTblNhanVien() {
        ...
        for (NhanVien nv : dsnv) {
            ...
            vec.add(nv.getGioiTinh());
            vec.add(nv.getChucVu());
            int trangThai = taiKhoanBLL.getTrangThai(nv.getMaNV() + "");
            if (trangThai == 0) {
                vec.add("Khoá");
            }
            else if(trangThai == 1) {
