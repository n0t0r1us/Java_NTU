...
import com.qlpz.BLL.CTPhieuNhapBLL;
import com.qlpz.BLL.PhieuNhapBLL;
import com.qlpz.DTO.PhieuNhap;
...
public class PnQuanLyNhapHangGUI extends javax.swing.JPanel {
    ...
    private CTPhieuNhapBLL ctPhieuNhapBLL = new CTPhieuNhapBLL();
    ...
    private void loadDataTableKho() {
        ...
        ArrayList<SanPham> dssp = sanPhamBLL.getListSanPham();
        for (SanPham sp : dssp) {
            if (sp.getMaLoai() != 1) {
    ...
    private void loadDataTableKho(String tuKhoa) {
        dtmKho.setRowCount(0);
        ArrayList<SanPham> dssp = sanPhamBLL.getSanPhamTheoTen(tuKhoa);
        for (SanPham sp : dssp) {
            if (sp.getMaLoai() != 1) {
                Vector vec = new Vector();
                vec.add(sp.getMaSP());
    ...
    private void loadDataCmbNhanVien() {
        ...
        ArrayList<NhanVien> dsnv = nhanVienBLL.getDanhSachNhanVien();
        if (dsnv != null) {
    ...
    private void loadDataTablePhieuNhap() {
        phieuNhapBLL.docDanhSach();
        ArrayList<PhieuNhap> dspn = phieuNhapBLL.getListPhieuNhap();
    ...
    private void loadDataTablePhieuNhapTheoGia(String giaThap, String giaCao) {
        phieuNhapBLL.docDanhSach();
        ArrayList<PhieuNhap> dspn = phieuNhapBLL.getListPhieuNhapTheoGia(giaThap, giaCao);
        duaDataVaoTablePhieuNhap(dspn);
    }

    private void loadDataTablePhieuNhapTheoNgay(String tuNgay, String denNgay) {
        phieuNhapBLL.docDanhSach();
        ArrayList<PhieuNhap> dspn = phieuNhapBLL.getListPhieuNhapTheoNgay(tuNgay, denNgay);
        duaDataVaoTablePhieuNhap(dspn);
    }

    private void loadDataTableCTPhieuNhap() {
        dtmCTPhieuNhap.setRowCount(0);
        ArrayList<CTPhieuNhap> dsct = ctPhieuNhapBLL.getListPhieuNhap();
        if (dsct != null) {
    ...
    private void loadDataTableCTPhieuNhap(String maPN) {
        dtmCTPhieuNhap.setRowCount(0);
        ArrayList<CTPhieuNhap> dsct = ctPhieuNhapBLL.getListPhieuNhap(maPN);
        if (dsct != null) {
            for (CTPhieuNhap ct : dsct) {
                Vector vec = new Vector();
                vec.add(ct.getMaSP());
    ...
    private void tblPhieuNhapMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblPhieuNhapMouseClicked
        ...
        PhieuNhap pn = phieuNhapBLL.timPhieuNhap(maPN);
    ...
    private void tblCTPhieuNhapMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblCTPhieuNhapMouseClicked
        ...
        String tenSP = sanPhamBLL.getTenSP(Integer.parseInt(tblCTPhieuNhap.getValueAt(row, 0) + ""));
